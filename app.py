...
# Load data
with open("projected_ko_olina_2025_2027.json", "r") as f:
    data = json.load(f)
...
# Correct Python date construction
...
for day in days:
    ...
    with col2:
        new_value = st.number_input(
            f"Points {day}",
            min_value=0,
            value=data[resort][unit_type][view_type].get(day, 0),
            key=day
        )
...
if st.button("Save to File") or updated:
    with open("projected_ko_olina_2025_2027_updated.json", "w") as f:
        json.dump(data, f, indent=2)
    if updated:
        st.success("Changes saved to projected_ko_olina_2025_2027_updated.json")
