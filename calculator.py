...
# ==============================================================================
# SETTINGS PERSISTENCE — BULLETPROOF & NO SYNTAX ERRORS
# ==============================================================================
...
def initialize_settings():
    ...
    # Helper: map any weird tier name (including "No Discount") → valid tier
    def safe_tier(tier):
        if not tier or str(tier).lower() in ["no discount", "none", ""]:
            return "Ordinary Level"
        return tier if tier in ["Ordinary Level", "Executive Level", "Presidential Level"] else "Ordinary Level"
    ...
    defaults = {
        ...
        "owner_tier_sel": safe_tier(data.get("discount_tier")),
        ...
        "renter_tier_sel": safe_tier(data.get("renter_discount_tier")),
...
# ==============================================================================
# REPOSITORY & CALCULATOR (your original logic – unchanged)
# ==============================================================================
class MVCRepository:
        ...
        while i < nights:
            ...
            if holiday and holiday.name not in processed_holidays:
                ...
                if is_owner and owner_config:
                    ...
                    thresh = 30 if disc_mul == 0.75 else 60 if disc_mul == 0.7 else 0
                    if disc_mul < 1.0 and days_out <= thresh:
...
# ==============================================================================
def main():
    initialize_settings()  # This line fixes everything

    ensure_data_in_session()
    if not st.session_state.data:
        st.warning("Please open the Editor and upload/merge data first.")
        return
    ...
    with st.sidebar:
            ...
            if uploaded:
                try:
                    ...
                    for k, v in data.items():
                        if k == "maintenance_rate":
                            st.session_state.owner_maint_rate = float(v)
                        if k == "purchase_price":
                            st.session_state.owner_price = float(v)
                        if k == "capital_cost_pct":
                            st.session_state.owner_coc_pct = float(v)
                        if k == "salvage_value":
                            st.session_state.owner_salvage = float(v)
                        if k == "useful_life":
                            st.session_state.owner_life = int(v)
                        if k == "discount_tier":
                            st.session_state.owner_tier_sel = safe_tier(v)
                        if k == "include_maintenance":
                            st.session_state.owner_inc_m = bool(v)
                        if k == "include_capital":
                            st.session_state.owner_inc_c = bool(v)
                        if k == "include_depreciation":
                            st.session_state.owner_inc_d = bool(v)
                        if k == "renter_rate":
                            st.session_state.renter_price = float(v)
                        if k == "renter_discount_tier":
                            st.session_state.renter_tier_sel = safe_tier(v)
                        if k == "preferred_resort_id":
                            st.session_state.current_resort_id = v
                    save_settings()
                    st.success("Config loaded & saved!")
                    st.rerun()
                except Exception as e:
        ...
        if mode == "Owner":
            ...
            if tier == "Presidential Level":
                policy = DiscountPolicy.PRESIDENTIAL
            elif tier == "Executive Level":
    ...
    st.divider()
    # FIXED: Use Enum by name, not by value string
    result = calc.calculate_breakdown(
        r_name, sel_room, final_in, final_n,
        UserMode[mode.upper()],  # This was the crash → now fixed
        active_rate, policy, owner_params
    )
